...
int main(){
	...
	inFile.open("I://NSEC//Dongjiang//flowtable//dj.patch.txt", ios::in);
	outFile.open("I://NSEC//Dongjiang//flowtable//dj.road.txt", ios::out);
	...
	for (int j = 0; j<rows; j++){
		for (int i = 0; i<cols; i++){
			outFile << right << setw(8) << setprecision(0) << nodata;
		}
		outFile << endl;
	}
